...
import java.util.Map;
 ...
 public class KpUtilities {
     ...
     public static ListKMIPAdapters listKmipAdapters(IbmKeyProtectApi testService, String testInstance, 
                long limit, long offset, boolean totalCount){
        // Build Request
        GetKmipAdaptersOptions getKmipAdapterModel = new GetKmipAdaptersOptions.Builder()
                .bluemixInstance(testInstance)
                .limit(limit)
                .offset(offset)
                ...
                .build();
        // execute API Call
        Response<ListKMIPAdapters> response = testService.getKmipAdapters(getKmipAdapterModel).execute();
        return response.getResult();
     }

     public static KMIPAdapter createKmipAdapter(IbmKeyProtectApi testService, String testInstance, 
                String profile, Map<String,String> profileData, String adapterName, String adapterDesc){
        // Construct JSON body
        InputStream inputstream = KpUtilities.buildCreateKMIPAdapterStream(profile, profileData, adapterName, adapterDesc);
        
        // Build Request
        CreateKmipAdapterOptions createKmipAdapterModel = new CreateKmipAdapterOptions.Builder()
                .bluemixInstance(testInstance)
                .body(inputstream)
                .build();
     ...
     public static void deleteKmipAdapter(IbmKeyProtectApi testService, String testInstance, 
                String adapterNameOrID){
        // Build Request
        DeleteKmipAdapterOptions deleteKmipAdapterModel = new GetKmipAdapterOptions.Builder()
                .bluemixInstance(testInstance)
                .id(adapterNameOrID)
                .build();
        // execute API Call
     ...
     public static InputStream buildCreateKMIPAdapterStream(String profile, Map<String,String> profileData, String adapterName, String description){        
        // build json format input stream
        JsonObjectBuilder profileDataBuilder = Json.createObjectBuilder();
        for (String dataKey : profileData.keySet()){
                profileDataBuilder.add(dataKey, profileData.get(dataKey));
        }
        
        JsonObjectBuilder resourceObjectBuilder = Json.createObjectBuilder()
                .add("profile", profile)
                .add("profile_data", profileDataBuilder);

        if (adapterName != null) {
                resourceObjectBuilder.add("name", adapterName);
        }
        if (description != null) {
                resourceObjectBuilder.add("description", description);
        }


        JsonObjectBuilder jsonObjectBuilder = Json.createObjectBuilder()
                 .add("metadata", Json.createObjectBuilder()
                         .add("collectionType", "application/vnd.ibm.kms.kmip_adapter+json")
                         .add("collectionTotal", 1))
                 .add("resources", Json.createArrayBuilder()
                         .add(resourceObjectBuilder));
     ...
     public static InputStream buildCreateKMIPCertStream(String certName, String certPayload){        
        // build json format input stream
        JsonObjectBuilder resourceObjectBuilder = Json.createObjectBuilder()
                .add("certificate", certPayload);

        if (certName != null) {
                resourceObjectBuilder.add("name", certName);
        }

        JsonObjectBuilder jsonObjectBuilder = Json.createObjectBuilder()
                 .add("metadata", Json.createObjectBuilder()
                         .add("collectionType", "application/vnd.ibm.kms.kmip_client_certificate+json")
                         .add("collectionTotal", 1))
                 .add("resources", Json.createArrayBuilder()
                         .add(resourceObjectBuilder));
